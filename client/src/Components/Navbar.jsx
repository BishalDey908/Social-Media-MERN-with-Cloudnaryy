...
const Navbar = () => {
  ...
  return (
    <div>
      <div id='navbar' className='w-full 2xl:h-24 xl:h-24 md:h-24  bg-cyan-700 flex fixed z-50 place-content-between '>
       
       <div  className='pt-6 flex py-6 2xl:py-0 xl:py-0 md:py-0'>
       <div className=''>
      <img src={friendbook} alt="" className='h-12 ml-6 ' />

      <div id='username2' className="mr-2 flex gap-2 ml-10">
                <a href="javascript:void(0)" className="dark:hover:text-primary hover:text-primary transition-colors duration-200 ease-in-out text-xl font-medium dark:text-white text-secondary-inverse font-body">{username ?`Welcome ${username} `: `Welcome user`}</a>
      ...
      <div id='searchSection' className='flex h-10  bg-white w-72 ml-6 py-2.5 rounded-full'>
       ...
       <div id='bell' className=' flex py-auto xl:gap-4 md:gap-4  pb-10 mr-10 mt-6 '>
    ...
    <div  className="p-2">
      ...
      >
        ...
        <div id='NavUsername' className="flex items-center pt-2  text-white">
            ...
            <div id='username1' className="mr-2 flex gap-2 ">
                <a href="javascript:void(0)" className="dark:hover:text-primary hover:text-primary transition-colors duration-200 ease-in-out text-xl font-medium dark:text-white text-secondary-inverse font-head">{username ?`Welcome ${username} `: `Welcome user`}</a>
                {/* <div><img src={love} className="w-6 pt-1 heartbeat" /></div> */}
            </div>
            </div> 
    ...
    <div  className="fixed z-10 inset-0 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-lg overflow-hidden shadow-xl max-w-screen-md w-full m-4 overflow-y-auto" >
        ...
        <div className="px-6 py-4">
          <h3 className="text-lg leading-6 font-bold text-gray-900 font-head "> Notification </h3>
        </div>
        <div className=' h-96 overflow-y-auto'>
        <hr />
        {
          loading ? notiFiData.map((item, index) => (
            <div key={index} >
            <div className='mx-10 my-4 flex justify-between font-body'>
            <div className='flex gap-2'>
              <img className='h-14 w-14 my-auto rounded-full' src={item.profilepic} alt=""  />
              <h1 className='text-2xl my-auto'>{item.message}</h1>
            </div>
              <div className='my-auto 2xl:flex '>
                <div type="button" className="text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2" onClick={()=>handleAccept(item)}> Accept</div>
        ...
        <div className="bg-gray-50 px-4 py-3 sm:px-6 flex align-items justify-end p-4 gap-4 flex-row font-body">
